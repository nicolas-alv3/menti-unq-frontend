  ...
  const handleDelete = (id) => () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        PresentationService.delete(id).then(() => {
          props.updateList();
          Swal.fire("¡Listo!", "La presentación ha sido eliminada", "success");
        });
      }
    });
  };
    ...
    <Paper sx={{ marginTop: "1em" }}>
      <Box sx={boxSx}>
        <Typography variant="h3">
          Presentaciones ({props.presentations.length})
        </Typography>
                ...
                <TableCell align="right">
                  ...
                  <IconButton aria-label="play">
                    <PlayArrowOutlinedIcon
                      onClick={() => navigate(`/presentar/${row.id}`)}
                    />
                  </IconButton>
                </TableCell>
              </TableRow>
