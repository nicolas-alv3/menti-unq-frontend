...
export function EditPresentationPanel({
  title,
  slides,
  setSlides,
  onSave,
  ...
  const handleSlideDeletion = (slideIndex) => {
    setSlides(
      slides
        .filter((_s, i) => i !== slideIndex)
        .map((s, i) => {
          return {
            ...s,
  ...
  const handleSortDown = (slideIndex) => {
    slides.splice(slideIndex + 1, 0, slides.splice(slideIndex, 1)[0]);
    setSlides(
      slides.map((s, i) => {
        return {
  ...
  return (
    <Paper sx={{ marginTop: "1em", padding: "1em 0" }}>
      ...
      >
        <ArrowBack
          fontSize="large"
          sx={{ ":hover": { cursor: "pointer", color: "#696969" } }}
          onClick={() => navigate(-1)}
        />
      ...
      <Box sx={{ height: "60vh", display: "flex" }}>
        <SlidesPanel
          ...
          slides={slides}
          deleteSlide={handleSlideDeletion}
